...
public class FileService {
            ...
            for (MultipartFile m : files) {
                if (!m.isEmpty()) {
                    ...
                    try {
                        ...
                        fileList.add(filePack);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
            }
            this.fileRepository.saveAllAndFlush(fileList);
        }
    }
    ...
    public void download(String savedPath, HttpServletResponse response) { // 파일 다운로드

        File file = new File(savedPath);
        savedPath = savedPath.substring(savedPath.lastIndexOf("_") + 1); // _와 _앞의 랜덤문자를 모두 자르고 나머지 파일명만 남김
    ...
    public void deleteFile(String dFile) {
        ...
        if(dFile.length()>0){
            ...
            List<Integer> fileNameList = Arrays.stream(dFiles).
                    map(file -> Integer.parseInt(file)).collect(Collectors.toList());

            this.fileRepository.deleteAllByIdInBatch(fileNameList); // id목록들로 삭제(한 번의 쿼리로 대량 삭제)
        }
    }
