...
public class BoardService {
	...
	public boolean pwdCheck(String password, Integer no) { // 비밀번호가 일치하면 해당 게시글을 수정하거나 삭제
																							// 가능
	...
	public void deleteAllByGrDepthGreaterThan(Integer no,Integer orNo, Integer depth) {	// 해당 답변의 하위답변들을 모두 삭제
		this.boardRepository.deleteAllByCondition(no,orNo, depth);
	}
	...
	/*
	 * public int getTotalPage(long totalElements,int displayRow) { int totalPage =
	 * (int)(totalElements/displayRow);
	 * 
	 * return (totalElements % displayRow) == 0 ? totalPage : totalPage+ 1; // 나머지가
	 * 있으면 1증가 }
	 * 
	 * public int getStartPage(int displayPage,int currentPage) { return
	 * ((currentPage - 1) / displayPage) * displayPage + 1; }
	 * 
	 * public int getEndPage(int startPage,int totalPage,int displayPage) { int
	 * endPage = startPage + displayPage - 1;
	 * 
	 * return endPage > totalPage ? totalPage : endPage; }
	 * 
	 * public boolean prev(int startPage) { return startPage==1? false : true; }
	 * 
	 * public boolean next(int endPage,int totalPage) { return endPage == totalPage?
	 * false : true; }
	 */
}
